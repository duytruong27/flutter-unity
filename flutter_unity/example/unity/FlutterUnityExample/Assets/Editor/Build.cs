...
public class Build
{
    ...
    {
        Debug.Log("iOS Path: " + iosExportPath);
        if (Directory.Exists(iosExportPath)) 
        {
            Debug.Log("Remove iOS Path");
    ...
    public static void OnPostProcessBuild(BuildTarget buildTarget, string buildPath) {
        if (buildTarget == BuildTarget.iOS) {
            ...
            var projPath = iosExportPath + "/Unity-iPhone.xcodeproj/project.pbxproj";
            var proj = new PBXProject();
            proj.ReadFromFile(projPath);

            var projectGuid = proj.ProjectGuid();

            //// Configure build settings
            proj.SetBuildProperty(projectGuid, "ENABLE_BITCODE", "NO");
            proj.SetBuildProperty(projectGuid, "OTHER_LDFLAGS", "-Wl,-U,_FlutterUnityPluginOnMessage");
            proj.SetBuildProperty(projectGuid, "CODE_SIGN_IDENTITY", "iPhone Developer");
